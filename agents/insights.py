...
def preflight_feasibility(
    ...
    Selling costs: agent 2.25% + marketing $3k
    ...
    agent_commission = arv_estimate * 0.0225
    marketing = 3_000
    selling_costs = agent_commission + marketing

    total_costs = reno_cost + buying_costs + holding_costs + selling_costs
    target_profit = arv_estimate * 0.15

    max_offer = arv_estimate - total_costs - target_profit
    actual_profit = arv_estimate - asking_price - total_costs
    actual_margin = (actual_profit / arv_estimate * 100) if arv_estimate > 0 else 0
    ...
    return {
        ...
        "target_profit_15pct":      round(target_profit),
...
Holding costs:           ${f['holding_costs']:,}  (5 months)
Selling costs:           ${f['selling_costs']:,}  (2.25% agent + $3k marketing)
Total costs (excl buy):  ${f['total_costs']:,}
─────────────────────────────────────────────────
Max offer for 15% margin: ${f['max_offer_price']:,}
Actual profit at asking:  ${f['actual_profit_at_asking']:,}
Actual margin at asking:  {f['actual_margin_pct']}%
...
{{
  ...
  "timing_recommendation": "<buy now|wait for price drop|monitor>",
  "negotiation_range": {{"low": <int>, "high": <int>}},
...
def print_analysis(analysis: dict):
    ...
    print(f"  Max offer:      ${analysis['feasibility']['max_offer']:,}")
    print(f"  Margin @ ask:   {analysis['feasibility']['margin_at_asking_pct']}%  ({analysis['feasibility']['verdict_at_asking']})")
    print(f"\n  Negotiation:    ${analysis.get('negotiation_range', {}).get('low', 0):,} – ${analysis.get('negotiation_range', {}).get('high', 0):,}")
    print(f"  Timing:         {analysis.get('timing_recommendation')}")

    print(f"\n  Scenarios:")
    for label, s in analysis.get("scenarios", {}).items():
        print(f"    {label.capitalize():6s}  reno ${s['reno_cost']:,}  →  profit ${s['profit']:,}  ({s['margin_pct']}%)")
